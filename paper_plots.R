...
dir.create("TIFF images", showWarnings = FALSE)
dir.create("PNG images", showWarnings = FALSE)
# --- patient data example
pat = "P09"

tiff("TIFF images/pat_example.tiff", 
...
png("PNG images/pat_example.png", 
     width=2250, height=900, units="px", res=300, pointsize=13,
    type="cairo")
{
  pat = "P09"
...
tiff("TIFF images/freq_example.tiff", 
     width=2250, height=900, units="px", res=300, pointsize=13,
     type="quartz")
{
  pat = "P09"
  ...
  op = par(cex.lab=1.3, cex.axis=1.3, mfrow=c(1,3), mar=c(4,4,1,0.5))
  for( chan in channels ){
    freq_class = as.data.frame( fread( paste0("./Output/frequentist_linReg/", chan, "__", pat, "__CLASSIF.txt") ) )
    freq_postpred = as.data.frame( fread( paste0("./Output/frequentist_linReg/", chan, "__POSTPRED.txt" )))
    ...
    manDef_cellID = mdat_pat[ mdat_pat$channel==chan & mdat_pat$classJOINT==1, "cell_id"]
...
png("PNG images/freq_example.png", 
     width=2250, height=900, units="px", res=300, pointsize=13,
     type="quartz")
{
  pat = "P09"
  colVec = c(alphaBlue(1.0), alphaRed(1.0))
  ...
  for( chan in channels ){
    ...
    points( data[data$sampleID%in% ctrlIDs & data$Channel==mitochan, "Value"],
            data[data$sampleID%in% ctrlIDs & data$Channel==chan, "Value"],
            pch=20, col=alphaBlack(0.05), bg=alphaBlack(0.05), cex=0.5)
    
    points( data[data$sampleID==pat & data$Channel==mitochan, "Value"],
            data[data$sampleID==pat & data$Channel==chan, "Value"],
            pch=20, col=colVec[freq_class[[1]]+1], bg=colVec[freq_class[[1]]+1],
            cex=0.5)
    ...
    points( log(manDef_x), log(manDef_y), pch=20, cex=0.3, col=alphaCol(alpha=1.0,"yellow"))
...
tiff("TIFF images/bayes_example.tiff", 
     width=2250, height=900, units="px", res=300, pointsize=13, 
     type="cairo")
{
  pat = "P09"
  op = par(cex.lab=1.3, cex.axis=1.3, mfrow=c(1,3), mar=c(4,4,1,0.5))
  for (chan in channels) {
    bayes_class_mat = as.matrix( fread( file.path(folder, outputFolder , paste0(chan, "__", pat, "__CLASSIF.txt") )) )
    bayes_class = colMeans( bayes_class_mat )
    
    bayes_postpred = as.data.frame(fread(file.path(folder, outputFolder, paste0(chan, "__", pat, "__POSTPRED.txt") )))
    
    xlm = range( data[data$sampleID%in% c(ctrlIDs, pat) & data$Channel==mitochan, "Value"] ) + c(0,0.5)
    ylm = range( data[data$sampleID%in% c(ctrlIDs, pat) & data$Channel!=mitochan, "Value"] ) + c(0,0.5)
    
    pi_est = round( colMeans( fread( file.path(folder, outputFolder, paste0(chan, "__", pat, "__POST.txt" )))[, "probdiff"] ), 3)
...
png("PNG images/bayes_example.png", 
     width=2250, height=900, units="px", res=300, pointsize=13, 
     type="cairo")
{
  pat = "P09"
...
tiff("TIFF images/prior_post.tiff", 
     ...
     type="cairo")
{
  ...
  post = as.data.frame( fread( file.path(folder, outputFolder, paste0(chan, "__", pat, "__POST.txt") ) ) )
  prior = as.data.frame( fread( file.path(folder, outputFolder, paste0(chan, "__", pat, "__PRIOR.txt") ) ) )
...
for (pat in ptsIDs) {
  for( chan in channels ){
    ...
    man_est = mean( unlist(mdat[ mdat$patient_id==pat & mdat$channel==chan, "classJOINT"]) )
...
tiff("TIFF images/proportion_comparison.tiff",
...
png("PNG images/proportion_comparison.png",
     width=2250, height=1900, units="px", res=300, pointsize=13,
     type="cairo")
{
  op = par(cex.lab=1.3, cex.axis=1.3, mar=c(4,4,1,0.5), xpd=FALSE)
...
tiff("TIFF images/gamma_comparison.tiff", 
...
png("PNG images/gamma_comparison.png", 
     width=2250, height=1000, units="px", res=300, pointsize=13, 
     type="cairo")
{
...
# --- prior comarison against OG posterior
...
for (pat in ptsIDs) {
  for (chan in channels) {
    ...
    fn_post_og = file.path(folder, outputFolder, paste0(root, "__POST.txt"))
    pi_post_og = as.matrix(fread(fn_post_og))[, "probdiff"]
    
    for (pp in names(folders)) {
      fn_post_two = file.path(folder, folders[pp], paste0(root, "__POST.txt"))
...
tiff("TIFF images/prior_comparison.tiff", 
...
png("PNG images/prior_comparison.png", 
     width=2250, height=1900, units="px", res=300, pointsize=13, 
     type="cairo")
{
  op = par( mfrow=c(2,1) )
...
tiff("TIFF images/allPost_classifs_1.tiff", 
     ...
     type="cairo")
{
  ...
  for (pat in ptsIDs[1:4]) {
    for (chan in channels) {
      root = paste0(chan, "__", pat)
      fn_post = file.path(folder, outputFolder, paste0(root, "__POST.txt"))
      fn_postpred = file.path(folder, outputFolder, paste0(root, "__POSTPRED.txt"))
      fn_classif = file.path(folder, outputFolder, paste0(root, "__CLASSIF.txt"))
...
tiff("TIFF images/allPost_classifs_2.tiff", 
     width=2250, height=2625, units="px", res=300, pointsize=13, 
     type="cairo")
{
  op = par(cex.lab=1.3, cex.axis=1.3, mfrow=c(4,3), mar=c(4,4,2,0.5), xpd=FALSE)
...
tiff("TIFF images/allPost_classifs_3.tiff", 
...
png("PNG images/allPost_classifs_1.png", 
     width=2250, height=2625, units="px", res=300, pointsize=13,
     type="cairo")
{
  op = par(cex.lab=1.3, cex.axis=1.3, mfrow=c(4,3), mar=c(4,4,2,0.5), xpd=FALSE)
...
png("PNG images/allPost_classifs_2.png", 
     width=2250, height=2625, units="px", res=300, pointsize=13, 
     type="cairo")
{
  op = par(cex.lab=1.3, cex.axis=1.3, mfrow=c(4,3), mar=c(4,4,2,0.5), xpd=FALSE)
...
png("PNG images/allPost_classifs_3.png", 
     width=2250, height=2625 *2/3, units="px", res=300, pointsize=13, 
     type="cairo")
{
  op = par(cex.lab=1.3, cex.axis=1.3, mfrow=c(3,3), mar=c(4,4,2,0.5), xpd=FALSE)
...
# --- wider priors n narrower priors

tiff("TIFF images/narrow_wide_priors.tiff", 
    ...
    type="cairo")
{
  ...
  pat = "P09"
  chan = "NDUFB8"
  file = paste0(chan, "__", pat, "__PRIOR.txt")
  fn_prior_n = file.path(folder, "Output_narrowPrior_new", file)
  fn_prior_w = file.path(folder, "Output_widePrior_new", file)
...
png("PNG images/narrow_wide_priors.png", 
    width=2250, height=600*3, units="px", res=300, pointsize=13, 
    type="cairo")
{
  op = par(mfrow=c(3,2), mar=c(4,4,1,0.5), cex.lab=1.3, cex.axis=1.3)
  ...
  for( pp in names(xlabels) ){
    ...
    plot(NA, ylab="Density", xlab=xlabels[[pp]],
         xlim=range(c(dens_n$x, dens_w$x)), 
         ylim=c(0, max(c(dens_n$y, dens_w$y))) )
    lines( dens_n, col="brown3" )
    lines( dens_w, col="darkolivegreen" )
  }
  
  plot(NA, xlim=c(0,1), ylim=c(0,1), axes=FALSE, xlab="", ylab="")
  legend("topleft", 
         legend=c("Narrow", "Wide"), lty=1, col=c("brown3", "darkolivegreen"), 
         box.col=alphaBlack(0.0), cex=1.3)
  par(op)
  
}
dev.off()

pdf("./pi_post_comparison.pdf", width=12, height=6)
{
  op = par(mfrow=c(1,3))
  for(pat in ptsIDs){
    for(chan in channels){
...
png("PNG images/diff_in_class.png", 
    width=2250, height=1200, units="px", res=300, pointsize=13, 
    type="cairo")
{
  pat = "P09"
  ...
  for( chan in channels ){
    man_class = mdat[mdat$Channel==chan & mdat$patient_id==pat, "classCONOR"]
    
    bayes_class_mat = as.matrix( 
      fread( 
        file.path(folder, outputFolder, paste0(chan, "__", pat, "__CLASSIF.txt"))))
    bayes_class = colMeans( bayes_class_mat )
    
    class_diff = man_class != as.numeric(bayes_class>0.5)
    
    bayes_postpred = as.data.frame( 
      fread( 
        file.path(folder, outputFolder, paste0(chan, "__", pat, "__POSTPRED.txt"))))
    
    xlm = range( data[data$sampleID%in% c(ctrlIDs, pat) & data$Channel==mitochan, "Value"] ) + c(0,0.5)
    ylm = range( data[data$sampleID%in% c(ctrlIDs, pat) & data$Channel!=mitochan, "Value"] ) + c(0,0.5)
    
    pi_est = round( colMeans( 
      fread( 
        file.path(folder, outputFolder, paste0(chan, "__", pat, "__POST.txt")))[,"probdiff"]),3)
