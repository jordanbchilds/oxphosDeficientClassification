...
outputFolder = file.path("..", "Data", "synData02")
parameterFolder = file.path("..", "Data", "trueParameters02.txt")

dir.create(outputFolder, showWarnings = FALSE)

dataIncreaseFactor = 1.5
dataShiftFactor = 4 # dataShiftFactor = 4

sdIncreaseFactor = 2 # sdIncreaseFactor = 2
...
for (patID in ptsIDs) {
  for (chan in channels) {
    ...
    # --- load posterior
    fn = file.path("..", "..", "OutputMaxESS", "stan_sampler", paste0(root, "__POST.txt"))
    post = fread(fn)
    ...
    patTau = ifelse(syntheticData[syntheticData$sampleID==patID & syntheticData$Channel==chan, "deficientStatus"], 
                    trueParameters[parameterIndex, "tau_def"], trueParameters[parameterIndex, "tau_norm"])
    ...
    write.table(syntheticData, file=file.path(outputFolder, paste0(root, "__", "synDATA.txt")),
